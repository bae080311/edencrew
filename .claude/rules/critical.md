...
6. 실시간 시세는 `SERVICE_ITEM:` 으로 **한 번에** 조회(종목별 반복 호출 금지). 일별 시세는 **EUC-KR**, `lastPage` 초과 요청 금지, **받은 페이지는 재사용**. 셋 다 평가 항목이다. **앱의 기본 동작은 실제 endpoint 조회이고 `USE_FAKE` 는 개발용 옵트인이다** — mock 에서 멈추면 미구현이다. → `data.md`
7. 커밋 전 `flutter analyze` 무경고 **와** `flutter test` 통과. **작업은 항상 새 브랜치에서 — `main` 직접 커밋 금지.** **자동 커밋 금지** — 사용자가 요청할 때만 커밋하고, 쌓이면 알려만 준다. 머지도 승인 후에만. → `conventions.md` · `testing.md`

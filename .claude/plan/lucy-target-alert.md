...
- [x] 튜토리얼 1회 따라하기 (처음이면 권장)

## 필수

- [x] 헤더에 `목표가 알림` 제목과 우측 `+` 버튼. `+` 는 **accent 색**
- [x] **등록된 목록** — 각 행에 좌측 **종목명**, 우측 **목표가**, 행 사이에 구분선
  - [x] 목표가는 천 단위 구분 쉼표 (예: `200,000`)
  - [x] **목표가 색이 매도/매수 구분** — 매수 `price/up/text`(빨강) · 매도 `price/down/text`(파랑). 시안이 한 목록에 둘을 섞어 보여준다
  - [ ] ~~**행은 컴포넌트로 분리해 재사용**~~ — **구현했다가 되돌렸다.** `AlertRow` 에 행 레이아웃 · 노출 변수 3 · 속성 바인딩 3 을 다 넣고 `ListView` 자식을 인스턴스로 교체까지 했으나, 프리뷰 런타임이 컴포넌트 인스턴스를 아예 렌더하지 않았다(캔버스에는 보임, Lucy Studio 0.0.34). 내부 위젯 id 는 밖에서 안 보이고(`txtName is not defined`) 인스턴스는 선언한 변수도 모른다(`getVar` → `undefined`). 값 주입 이전에 렌더가 안 되는 문제라 인라인 행으로 복구했다. `AlertRow.lfc` 는 완성 상태로 남겨 뒀고 사유는 README 에 적었다.
- [x] **빈 상태** — 항목이 하나도 없을 때 아이콘, `등록된 알림이 없습니다`, `우측 상단 + 버튼을 눌러 목표가를 추가해보세요`. 이때도 헤더 유지
- [x] **등록 다이얼로그** — `+` 버튼으로 열림
  - [x] 제목 `목표가 알림 추가`
  - [x] 입력 항목 `종목명`, `목표가` 두 개, 각각 위에 라벨
  - [x] `목표가` placeholder 는 `숫자만 입력`
  - [x] 우측 하단에 `매도` 와 `매수` 버튼. **두 버튼 모두 등록**하며, 어느 쪽을 눌렀는지가 항목의 구분이 됨
  - [x] 닫기는 우측 상단 `x` 아이콘
  - [x] 다이얼로그가 열려 있을 때 뒤 화면이 어둡게 가려짐
- [x] **등록하면 입력한 항목이 리스트에 실제로 추가된다** (그려두기만 하면 부족, 눌러서 동작해야 함)
- [x] `x` 를 누르면 아무것도 추가되지 않고 닫힘
- [x] 빈 값 검증 (필수는 여기까지)
...
- [x] `목표가` 입력을 숫자만 받게 제한할지, 등록 시점에 걸러낼지 — 초안은 **등록 시점에 숫자 아닌 문자를 걸러내는 것**(`replace(/[^0-9]/g, "")`). 입력 중에 막으면 붙여넣기가 불편하다
- [x] 빈 값일 때 무엇을 보여줄지 — 초안은 **닫지 않고 그대로 둔다**(에러 문구 없음). 시안에 에러 상태가 없다
...
- [x] 페이지 이름이 `targetAlert` 인지 재확인
- [x] `cloneProject/assets` 를 한 번에 압축 → `~/edenCrew/lucy/` 에 보관
- [x] `unzip -l` 로 내용 확인

...
- [x] **등록된 목록** — 각 행에 좌측 **종목명**, 우측 **목표가**, 행 사이에 구분선
  ...
  - [ ] ~~**행은 컴포넌트로 분리해 재사용**~~ — **못 했다.** `AlertRow` 컴포넌트와 노출 변수 설계까지 마쳤지만, 컴포넌트 변수가 리스트의 행 스코프를 타는지 확인하는 데 시간을 다 쓰면 "등록하면 실제로 추가된다"는 동작 요구를 못 채울 위험이 컸다. 행을 ListView 의 반복 자식으로 직접 두는 쪽을 택했고 사유를 README 에 남겼다. 컴포넌트 파일은 지우지 않았다.

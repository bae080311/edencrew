...
- [x] **검색 결과 행 → 상세 이동** — Phase 2 에서 미룬 **필수** 항목. `DetailView` 가 생기면 `SearchRow` 에 `onTap` 을 붙이고 `Navigator.push` 로 연다.
- [x] 헤더 — 뒤로 가기, 종목명, `종목코드 · 시장`, 관심 버튼(여기서 해제하고 돌아오면 목록 반영되는지 확인).
- [x] 현재가 + 등락(▲/▼ 아이콘).
- [x] 요약 카드 — 시가·고가·저가·거래량·시가총액(축약).
- [x] 일별 시세 표 — `날짜`(MM.DD) · `종가` · `등락`(부호+색) · `거래량`.
- [x] **기간 탭 4개 동작** — 탭 전환은 latest-wins. **필요한 페이지만 요청하고 받은 페이지는 재사용**(평가 항목이므로 실제로 요청 수를 세어 확인).
- [x] 캔들 차트 — `CustomPainter` 로 직접. 상승/하락 색(`chartLineUp`/`chartLineDown`)만 맞추면 필수 충족. 패키지를 쓸 경우 시안과 달라진 점을 README 에.
- [x] `flutter analyze` 정리, `flutter test` 실행.
- [x] README 채우기 — 기술 선택과 이유, 구현 범위, 막혔던 지점.

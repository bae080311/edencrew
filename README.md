...
- **`Scale` 컬렉션에 없는 시안 값도 `AppDimens` 에 추가해서 쓴다.** 행 내부 간격 2, 탭 라벨 간격 3, 아이콘 22 · 24 · 40, 바텀시트 라운드 16 · 제목 영역 높이 64가 그렇다. 처음에는 대응표가 깨지는 게 싫어 위젯 안에 `const` 로 뒀는데, 그러면 같은 값이 화면마다 흩어져 "간격은 토큰으로만" 규칙이 무의미해진다. 대응표는 Figma 변수에서 온 것과 시안 레이어에서 읽은 것을 표로 갈라 적는 쪽으로 해결했다([`theme/README.md`](app/lib/theme/README.md)).

...
- **`Scale` 컬렉션에 없는 시안 값은 토큰으로 올리지 않고 쓰는 위젯 안에 `const` 로 둔다.** 행 내부 간격 2, 탭 라벨 간격 3, 아이콘 22 · 24 · 40, 바텀시트 라운드 16 · 제목 영역 높이 64, 스켈레톤 막대 크기가 그렇다. `AppDimens` 에 넣으면 대응하는 Figma 변수가 없는 필드가 섞여 "Figma 변수 ↔ Dart 필드" 1:1 대응표가 깨진다. 토큰에 없는 값은 시안에서 직접 읽어 쓴다는 `theme/README.md` 의 폰트 크기 방침과 같은 처리다.
